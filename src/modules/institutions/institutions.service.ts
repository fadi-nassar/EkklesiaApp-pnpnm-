...
//for super admin only
  ...
  async assignAdminToInstitution(institutionId: string, adminId: string): Promise<Institution> {
    ...
    try {
      const institution = await this.institutionModel.findById(institutionId).session(session);
      if (!institution) {
      ...
      if (admin.role === 'superAdmin') {
        throw new BadRequestException(`User with ID ${adminId} is a superAdmin and cannot be assigned as a churchAdmin.`);
      }

      const adminObjectId = new Types.ObjectId(adminId);
      if (institution.admins.some((existingAdminId) => existingAdminId.equals(adminObjectId))) {
        throw new BadRequestException(`User with ID ${adminId} is already an admin of this institution.`);
      }
      if (institution.admins.length >= 5) {
        throw new BadRequestException(`Institution with ID ${institutionId} already has the maximum of 5 admins.`);
      }

      institution.admins.push(adminObjectId);

      admin.role = 'churchAdmin';
      if (!admin.managedInstitutionIds.some((id) => id.equals(institution._id as Types.ObjectId))) {
 ...
 //for anyone
  async getInstitutionById(id: string): Promise<Institution> {
    const institution = await this.institutionModel.findById(id).select('-admins').exec();
    if (!institution) {
  ...
  async getAllInstitutionsByNameTypeRite(query: { name?: string; type?: string; rite?: string }): Promise<Institution[]> {
    const filter: any = {};
    if (query.name) {
